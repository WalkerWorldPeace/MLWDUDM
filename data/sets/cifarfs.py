...
class CIFARFS(object):
    """
    Dataset statistics:
    # 64 * 600 (train) + 16 * 600 (val) + 20 * 600 (test)
    """

    def __init__(self, dataset, dataset_dir, epoch):
        super(CIFARFS, self).__init__()
        self.dataset_dir = dataset_dir
        self.train_dir = os.path.join(os.getcwd(),f'sync_image/{dataset}/{epoch}')
        self.val_dir = os.path.join(self.dataset_dir, 'meta_val')
        self.test_dir = os.path.join(self.dataset_dir, 'meta_test')

        self.train, self.train_labels2inds, self.train_labelIds = self._process_dir(self.train_dir)
        self.val, self.val_labels2inds, self.val_labelIds = self.p_process_dir(self.val_dir)
        self.test, self.test_labels2inds, self.test_labelIds = self.p_process_dir(self.test_dir)
    ...
    def _process_dir(self, dir_path):
        ...
        for cat in cat_container:
            if cat == "log.txt" or cat == "progress.csv":
                continue
    ...
    def p_process_dir(self, dir_path):
        cat_container = sorted(os.listdir(dir_path))
        cats2label = {cat:label for label, cat in enumerate(cat_container)}
